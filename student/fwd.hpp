...
uint32_t const maxAttributes = 4;///< maximum number of points/fragment attributes
...
/**
 * @brief This enum represents points/fragment attribute type.
 */
//! [AttributeType]
...
/**
 * @brief This union represents one points/fragment attribute
 */
//! [Attribute]
union Attribute{
...
/**
 * @brief This struct represents input points of points shader.
 */
//! [InVertex]
struct InVertex{
  Attribute attributes[maxAttributes]    ; ///< points attributes
  uint32_t  gl_VertexID               = 0; ///< points id
  uint32_t  gl_DrawID                 = 0; ///< draw id
};
//! [InVertex]

/**
 * @brief This struct represents output points of points shader.
 */
//! [OutVertex]
struct OutVertex{
  Attribute attributes[maxAttributes]                     ; ///< points attributes
...
/**
 * @brief Function type for points shader
 *
 * @param outVertex output points
 * @param inVertex input points
 * @param uniforms uniform variables
...
/**
 * @brief This struct describes location of one points attribute.
 */
//! [VertexAttrib]
struct VertexAttrib{
...
/**
 * @brief This structure represents setting for points pulller (points assembly) unit.
 * VertexArrays holds setting for reading vertices from buffers.
 */
//! [VertexArray]
struct VertexArray{
  VertexAttrib vertexAttrib[maxAttributes];       ///< settings for points attributes
...
//! [Program]
struct Program{
  VertexShader   vertexShader   = nullptr; ///< points shader
  FragmentShader fragmentShader = nullptr; ///< fragment shader
  AttributeType  vs2fs[maxAttributes] = {AttributeType::EMPTY}; ///< which attributes are interpolated from points shader to fragment shader
};
//! [Program]
...
struct DrawCommand{
  ...
  VertexArray vao                    ; ///< active points array (input/ triangles)
};
//! [DrawCommand]
...
struct Mesh{
  ...
  IndexType    indexType      = IndexType::UINT32;///< type of indices
  VertexAttrib position                          ;///< position points attribute
  VertexAttrib normal                            ;///< normal points attribute
  VertexAttrib texCoord                          ;///< tex. coord points attribute
