...
void triangleRasterizationTest(glm::vec4 const&a,glm::vec4 const&b,glm::vec4 const&c,glm::uvec2 const&res,bool backFaceCulling){
  ...
  inFragments.clear();
  auto&outVertices = dumpInject.outVertices;
  dumpInject.outVertices.clear();
  dumpInject.outVertices.resize(3);
  dumpInject.outVertices[0].gl_Position=a;
  dumpInject.outVertices[1].gl_Position=b;
  dumpInject.outVertices[2].gl_Position=c;
  ...
  pushDrawCommand(cb,3,0,{},backFaceCulling);
  ...
  if(!success){
    ...
    Vrchol co vyleze z points shaderu (x,y,z,w)
    ...
    Pozor na backFaceCulling, pokud je zapnutý (ctx.backFaceCulling==true),
...
SCENARIO("10"){
  std::cerr << "10 - rasterization should produce correct number of fragments" << std::endl;
...
SCENARIO("11"){
  std::cerr << "11 - perspective division" << std::endl;
  ...
  if(!success){
    ...
    Z points shaderu vycházejí vrcholy, které mají homogenní složku jinou od 1.
...
SCENARIO("12"){
  std::cerr << "12 - gl_FragCoord.xy should be inside triangle" << std::endl;
...
SCENARIO("13"){
  std::cerr << "13 - depth interpolation" << std::endl;
...
SCENARIO("14"){
  std::cerr << "14 - points attributes interpolated to fragment attributes" << std::endl;
  ...
  if(!success){
    std::cerr << R".(
    Tento test zkouší, zda se interpolují points attributy do fragment attributů.
...
SCENARIO("15"){
  std::cerr << "15 - perspective correct interpolation of points attributes to fragment attributes" << std::endl;
  ...
  if(!success){
    std::cerr << R".(
    Tento test zkouší, zda se perspektivně správně interpoluje points attributy.
...
SCENARIO("16"){
  std::cerr << "16 - fragment color should be written to framebuffer" << std::endl;
  ...
  mem.framebuffer = framebuffer->getFrame();
  mem.programs[0].vertexShader = vertexShaderInject;
  mem.programs[0].fragmentShader = fragmentShaderWhite;
  ...
  for(uint32_t y=0;y<res.y;++y)
    for(uint32_t x=0;x<res.x;++x){
      ...
      auto color = readColor(mem.framebuffer,coord);
      bool good;
...
SCENARIO("17"){
  std::cerr << "17 - fragment depth should be written to framebuffer" << std::endl;
  ...
  auto&depth = mem.framebuffer.depth;
  for(uint32_t y=0;y<res.y;++y)
    for(uint32_t x=0;x<res.x;++x){
      auto pix = y*res.x+x;
      bool good;
...
SCENARIO("18"){
  std::cerr << "18 - depth test" << std::endl;
  ...
  mem.framebuffer = framebuffer->getFrame();
  mem.programs[0].vertexShader   = vertexShaderInject;
  mem.programs[0].fragmentShader = fragmentShaderDepthTest;
  ...
  success &= mem.framebuffer.color[(0*res.x+0)*4+0] == 0;
  success &= mem.framebuffer.color[(0*res.x+0)*4+1] == 255;
  success &= mem.framebuffer.color[(0*res.x+0)*4+2] == 0  ;

  success &= mem.framebuffer.color[((res.y-1)*res.x+res.x-1)*4+0] == 255;
  success &= mem.framebuffer.color[((res.y-1)*res.x+res.x-1)*4+1] == 0  ;
  success &= mem.framebuffer.color[((res.y-1)*res.x+res.x-1)*4+2] == 0  ;
...
SCENARIO("19"){
  ...
  auto finalColor    = readColor(mem.framebuffer,samplePos);
...
SCENARIO("20"){
  std::cerr << "20 - blending + depth" << std::endl;
  ...
  auto frameColor = glm::vec4(.4f,.3f,.2f,1);
  auto frameDepth = 1.f;
  ...
  pushClearCommand(cb,frameColor,frameDepth);
  ...
  auto finalColor1 = readColor(mem.framebuffer,samplePos1);
  auto finalColor2 = readColor(mem.framebuffer,samplePos2);

  auto expectedColor1 = alphaMix(frameColor,color1);
  auto expectedColor2 = alphaMix(frameColor,color2);
  auto finalDepth1    = readDepth(mem.framebuffer,samplePos1);
  auto finalDepth2    = readDepth(mem.framebuffer,samplePos2);
